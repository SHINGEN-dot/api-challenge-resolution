...
logger_api = loggerApi.get_logger()
...
def catch_exceptions(function):
    ...
    def decorator(*args, **kwargs):
        try:
            ...
            logger_api.error("%s -- function %s -- args %s --kwargs %s -- line %s: %s",
...
def catch_exceptions_and_performance(function):
    ...
    def decorator(*args, **kwargs):
        try:
            ...
            logger_api.info("%s-%s-%d ms", os.path.split(function.__code__.co_filename)[1],
                            function.__name__, execution_time)
            return result
        except Exception as error:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            logger_api.error("%s -- function %s -- line %s: %s",
                             os.path.split(traceback.extract_tb(exc_tb)[-1][0])[1],
